...
/* ── Logo: cargado desde assets/ con fallback a base64 embebida ─── */
// Prioridad 1: assets/logo.jpg  (JPEG nativo en jsPDF, sin conversión)
// Prioridad 2: assets/logo.png  (PNG → canvas → JPEG)
// Prioridad 3: LOGO_B64 embebida (fallback de emergencia)
...
// logoJpeg: base64 JPEG puro (sin prefijo), listo para doc.addImage(data,'JPEG',...)
...
function loadKanban() {
  ...
  STAGES.forEach(stage => {
    ...
    const cardsHtml = stageRecs.length === 0
    ...
    col.innerHTML = `
      ...
      <div class="kanban-cards" id="kc-${stage.replace(/ /g,'_')}">${cardsHtml}</div>`;

    // ── Drag-and-drop para la columna ────────────────────────────
    ...
    cardsEl.addEventListener('dragleave', e => {
      // Solo eliminar si el destino está fuera del contenedor
    ...
    cardsEl.addEventListener('drop', e => {
      e.preventDefault();
      cardsEl.classList.remove('drag-over');
      if (draggedKanbanId !== null) {
        onKanbanDrop(draggedKanbanId, stage);
...
function onKanbanDragStart(e, id) {
  ...
  // Pequeño timeout para que el cursor no cambie antes de que se inicie el drag
  setTimeout(() => {
...
function onKanbanDragEnd(e) {
  // Eliminar clase de arrastre y limpiar estado
...
async function onKanbanDrop(id, newStage) {
  const rec = allRecords.find(r => r.id === id);
  if (!rec) return;
  if ((rec.estado ?? 'Recibida') === newStage) return; // Sin cambio

  showLoading('Actualizando estado…');
  const { error } = await db.from('reportes_pqrsf').update({ estado: newStage }).eq('id', id);
  hideLoading();
...
function buildKanbanCard(r) {
  ...
  <div class="kanban-card"
       draggable="true"
       data-id="${r.id}"
       ondragstart="onKanbanDragStart(event, ${r.id})"
...
/* ════════════════════════════════════════════════════════════
   ADMIN MODULE
════════════════════════════════════════════════════════════ */
let adminCurrentTab  = 'perfiles';
let adminEditingId   = null;      // ID del registro que se edita en el modal
let adminModalSaveFn = null;      // función que se llama al guardar

function loadAdminSection() {
  showAdminTab(adminCurrentTab || 'perfiles');
}
...
/* ── Usuarios / Perfiles ──────────────────────────────────── */
async function loadAdminPerfiles() {
  const panel = document.getElementById('adminContent');
  panel.innerHTML = '<div class="admin-empty"><i class="fa-solid fa-spinner fa-spin"></i> Cargando usuarios…</div>';
  ...
  if (error) {
    panel.innerHTML = `<div class="admin-empty" style="color:#dc2626">Error: ${esc(error.message)}</div>`;
    return;
  }

  const rows = (data ?? []).map(u => `
    <tr>
      <td style="font-weight:700;color:#0d2d6b;font-size:12px">${esc(u.id?.substring(0,8))}…</td>
      <td>${esc(u.email ?? '—')}</td>
      <td>${esc(u.nombre ?? '—')}</td>
      <td><span class="badge-rol-${u.rol === 'admin' ? 'admin' : 'analista'}">${esc(u.rol ?? 'analista')}</span></td>
      <td style="font-size:12px;color:#6b7280">${u.created_at ? u.created_at.split('T')[0] : '—'}</td>
      <td>
        <div class="table-actions">
          <button class="btn-tbl view" onclick="adminEditPerfil('${u.id}')" title="Editar"><i class="fa-solid fa-pen"></i></button>
        </div>
      </td>
    </tr>`).join('');
  ...
  panel.innerHTML = `
    <div class="admin-panel-header">
      <span style="font-size:14px;font-weight:700;color:#0d2d6b"><i class="fa-solid fa-users" style="margin-right:6px"></i>Usuarios del sistema</span>
      <button class="btn-primary-sm" onclick="adminNewPerfil()"><i class="fa-solid fa-plus"></i> Nuevo usuario</button>
    </div>
    <div class="admin-table-wrap">
      <table class="admin-table">
        <thead>
          <tr>
            <th>ID (primeros 8)</th><th>Correo</th><th>Nombre</th><th>Rol</th><th>Creado</th><th>Acciones</th>
          </tr>
        </thead>
        <tbody>${rows || '<tr><td colspan="6" class="admin-empty">Sin perfiles registrados</td></tr>'}</tbody>
      </table>
    </div>
    <div style="margin-top:12px;padding:12px;background:#fef9c3;border-radius:8px;font-size:12px;color:#78350f">
      <i class="fa-solid fa-circle-info" style="margin-right:4px"></i>
      Para crear un usuario en Auth (correo + contraseña), use el panel de Supabase → Authentication → Users → Invite user.
      Luego ingrese aquí los datos del perfil con el mismo ID de Auth.
    </div>`;
}
  ...
  db.from('consola_perfiles').select('*').eq('id', id).single().then(({ data: u }) => {
    ...
    document.getElementById('adminModalBody').innerHTML = `
      <div class="admin-form-grid">
        ...
        <div class="admin-form-field">
          <label>Nombre completo</label>
          <input id="ap-nombre" value="${esc(u.nombre ?? '')}"/>
        </div>
        <div class="admin-form-field">
          <label>Correo electrónico</label>
          <input id="ap-email" type="email" value="${esc(u.email ?? '')}"/>
        </div>
        <div class="admin-form-field">
          <label>Rol</label>
          <select id="ap-rol">
...
/* ── Función genérica invocada por el botón Guardar del modal ── */
function adminModalSave() {
  if (typeof adminModalSaveFn === 'function') adminModalSaveFn();
}
