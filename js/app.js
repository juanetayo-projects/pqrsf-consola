...
/* ── Logo: precargado como HTMLImageElement (data URI → canvas → jsPDF) ── */
// jsPDF acepta HTMLImageElement directamente y lo renderiza vía canvas,
// evitando por completo su parser interno de PNG que fallaba con este archivo.
// Con una data URI, img.complete = true de forma síncrona → listo al instante.
...
// HTMLImageElement precargado — data URI carga de forma síncrona en el navegador
const LOGO_IMG = new Image();
LOGO_IMG.src = LOGO_B64;   // .complete = true inmediatamente (no es HTTP, no espera red)

let logoBase64 = LOGO_B64; // mantenido por compatibilidad

/* ── preloadLogo: no-op, todo ya está listo ─────────────────── */
async function preloadLogo() { /* logo incrustado, nada que cargar */ }
...
function generatePDFById(id) {
  ...
  // ── Logo header ───────────────────────────────────────────────
  // Dimensiones fijas basadas en el PNG real (575×677 px, ratio ≈ 0.85)
  const logoH = 28;
  const logoW = logoH * (575 / 677); // ≈ 23.8 mm

  // LOGO_IMG es un HTMLImageElement ya cargado con data URI.
  // jsPDF lo renderiza vía canvas interno → no usa su parser PNG propio.
  if (LOGO_IMG.complete && LOGO_IMG.naturalWidth > 0) {
    doc.addImage(LOGO_IMG, 'PNG', ml, (38 - logoH) / 2, logoW, logoH);
    const tx = ml + logoW + 5;
    ...
    // Fallback: texto centrado (no debería ocurrir con data URI)
    doc.setTextColor(255, 255, 255);
